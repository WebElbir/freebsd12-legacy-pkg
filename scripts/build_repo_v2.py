...
def shreq(c):
    # "shlibs" is the legacy spelling for shlibs_required.
...
def version_key(v):
    # Deterministic approximation of pkg-version ordering used only as a final
    # tie-breaker after runtime coverage/source coherence/exact provenance.
    out = []
...
def local12_v2(target, priority):
    """Reuse either the original root-level FreeBSD 12 repo or a prior v2 run."""
    candidates = []

    old_all = core.ROOT / "All"
    old_cat = next((core.ROOT / x for x in ("packagesite.pkg", "packagesite.txz")
                    if (core.ROOT / x).exists()), None)
    if old_all.is_dir() and old_cat:
        candidates.append((old_all, old_cat))

    new_root = core.ROOT / target / "latest"
    new_all = new_root / "All"
    new_cat = next((new_root / x for x in ("packagesite.pkg", "packagesite.txz")
                    if (new_root / x).exists()), None)
...
def _candidate_score(c, parents):
    ...
    exact_provenance = 0
    same_snapshot = 0
    for parent, wanted_version in parents:
        runtime_coverage += len(provided & shreq(parent))
        if wanted_version and c["version"] == wanted_version:
            exact_provenance += 1
        if c["source"] == parent["source"]:
            same_snapshot += 1
    # Runtime ABI is authoritative. After that prefer the candidate that came
    # from most parent snapshots, then exact historical version references,
    # then the newest package version as a deterministic union-repo tie-break.
    return (runtime_coverage, same_snapshot, exact_provenance,
            version_key(c["version"]), -int(c["priority"]))
...
def _runtime_validation(selected, all_rows):
    ...
    for c in selected.values():
        for lib in shreq(c):
            # If no package in any source advertises this SONAME, it is assumed
            # to be supplied by the FreeBSD base system rather than our repo.
            if lib in known_providers and lib not in provided:
                missing[lib].append(c)

    report = []
    for lib, parents in sorted(missing.items()):
        providers = known_providers[lib]
        report.append({
            "soname": lib,
            "required_by": sorted({p["name"] + "-" + p["version"] for p in parents}),
            "available_providers": sorted({p["name"] + "-" + p["version"] + "@" + p["source"]
                                           for p in providers}),
        })
    return report
...
def select_v2(rows, cfg):
    ...
    if gcc.get("include_generic"):
        requested.add(gcc.get("generic_name", "gcc"))
    ...
    for name in sorted(requested):
        ...
        if not opts:
            roots.append({"name": name, "status": "unresolved", "attempts": []})
            continue
        c = opts[0]
        root_selected[name] = c
        roots.append({"name": name, "status": "selected", "version": c["version"],
                      "source": c["source"]})

    selected = dict(root_selected)
    version_notes = []

    # Recompute the complete closure until candidate choices stabilize. This
    # also prunes dependencies that belonged only to a superseded candidate.
    for _ in range(64):
            ...
            for dep_name, wanted_version in core.deps(parent):
                ...
                if dep_name not in byname:
                    unresolved_deps.append({
                        "package": dep_name,
                        "required_by": parent["name"],
                        "required_version": wanted_version,
                    })

        if unresolved_deps:
            sample = unresolved_deps[:20]
            raise RuntimeError("unresolved runtime dependencies: " + json.dumps(sample, sort_keys=True))

        new_selected = dict(root_selected)
        notes = []
        for dep_name, parents in requirements.items():
            if dep_name in root_selected:
                ...
                if not opts:
                    continue
                chosen = max(opts, key=lambda c: _candidate_score(c, parents))
                new_selected[dep_name] = chosen
            ...
            if wanted and (len(wanted) > 1 or chosen["version"] not in wanted):
                notes.append({
                    ...
                    "historical_versions": wanted,
                    "reason": "union repo selected one runtime candidate; dependency version is provenance, SONAME validation is authoritative",
                })

        old_sig = {(n, c["version"], c["source"]) for n, c in selected.items()}
        new_sig = {(n, c["version"], c["source"]) for n, c in new_selected.items()}
    ...
    if missing:
        # A union catalogue has one active package record per package name. If
        # different historical roots need mutually incompatible SONAMEs from
        # different versions of that same dependency, publishing this union as
        # healthy would be wrong. Stop and report the exact incompatibility.
